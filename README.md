Video Explanation: https://drive.google.com/file/d/1D8aSiCpjBzF7hIAdRSLF0kEjlJZoxqHF/view?usp=sharing
...
So the starting point we want to use for `P` or the `positionOfPiece` is `(width/2, 0)`, or in other words, `(5,0)`.  
This value will act as an anchor point, and starts the game pieces at the top center of the grid, which is where we want our pieces to spawn.
...
If you wanted to `MovePiece(0,1)`, or in other words move the piece down one space from the starting position, we would need an input to update the anchor point of the piece to be `(5,1)` + `[(0,0), (-1,0), (1,0), (0,1)]`, which results in:
```
[5, 1],       // center
[4, 1],       // left
[6, 1],       // right
...
Likewise, if you wanted to `MovePiece(1,0)`, or in other words move the piece to the right one space from the starting position, we would need an input to update the anchor point to be `(6,0)` + `[(0,0), (-1,0), (1,0), (0,1)]`, resulting in:
```
[6, 0],       // center
[5, 0],       // left
[7, 0],       // right
...
{
    by using the input values, as well as the values foreach (of the offset blocks on the x and y axis that make up the tetromino shape)  
    {
        ---
        ...
        ---
        then we can use the values of int x and int y to determine if (the values of the x axis of the tetris piece are outside the grid on the left or the right,
        or if the sum of the values of y are outside the top or the bottom of it, or if the piece is within the grid but the cell is already occupied (!= 0, 1 means occupied)
        ---
